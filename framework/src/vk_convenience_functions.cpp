...
namespace gvk
{
	...
	vk::Format default_depth_format() noexcept
	{
		...
		if (gvk::context().state() < context_state::fully_initialized) {
		...
		for (size_t i = 0; i < formatCandidates.size(); ++i) {
			auto formatProps = gvk::context().physical_device().getFormatProperties(formatCandidates[i]);
	...
	{
		if (nullptr == aWindow) {
			aWindow = gvk::context().main_window();
		}
