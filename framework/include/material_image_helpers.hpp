...
namespace gvk
{
	...
	*/
	...
	template <typename... Rest>
	void add_tuple_or_indices_shared(std::vector<std::tuple<avk::resource_ownership<gvk::model_t>, std::vector<mesh_index_t>>>& aResult, model& aModel, Rest&... rest)
	{
		aResult.emplace_back(avk::shared(aModel), std::vector<size_t>{});
		add_tuple_or_indices_shared(aResult, rest...);
	}

	template <typename... Rest>
	void add_tuple_or_indices_shared(std::vector<std::tuple<avk::resource_ownership<gvk::model_t>, std::vector<mesh_index_t>>>& aResult, size_t aMeshIndex, Rest&... rest)
	{
		std::get<std::vector<size_t>>(aResult.back()).emplace_back(aMeshIndex);
		add_tuple_or_indices_shared(aResult, rest...);
	}
