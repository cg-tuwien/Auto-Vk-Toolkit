...
public: // v== avk::invokee overrides which will be invoked by the framework ==v
	...
	void initialize() override
	{
		...
		auto animated_tube = gvk::model_t::load_from_file("assets/skinning_test_tube_animation.fbx", aiProcess_Triangulate | aiProcess_PreTransformVertices);
		...
		for (const auto& pair : distinctMaterials) {
			...
			for (auto index : pair.second) {
				gvk::append_indices_and_vertex_data(
					gvk::additional_index_data(newElement.mIndices, [&]() { return animated_tube->indices_for_mesh<uint32_t>(index);					}),
					gvk::additional_vertex_data(newElement.mPositions, [&]() { return animated_tube->positions_for_mesh(index);							}),
					gvk::additional_vertex_data(newElement.mTexCoords, [&]() { return animated_tube->texture_coordinates_for_mesh<glm::vec2>(index, 0);	}),
					gvk::additional_vertex_data(newElement.mNormals, [&]() { return animated_tube->normals_for_mesh(index);								})
				);
			}
