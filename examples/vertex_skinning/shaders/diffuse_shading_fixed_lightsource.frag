...
void main() 
{
	...
	vec3 toLight2 = normalize(vec3(-1.0, 0.5, 0.5));
	vec3 firstLightColor = diffuse * max(0.0, dot(normalize(normalWS), toLight)) * vec3(0.3, 0.7, 1.0);
	vec3 secondLightColor = diffuse * max(0.0, dot(normalize(normalWS), toLight2)) * vec3(1.0, 0.3, 0.1);
	vec3 illum = vec3(ambient) + firstLightColor + secondLightColor;
	color *= illum;
