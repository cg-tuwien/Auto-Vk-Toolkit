...
public: // v== cgb::cg_element overrides which will be invoked by the framework ==v
	...
	{
		std::vector<cgb::bottom_level_acceleration_structure> bottomLevelAccStructures;
		...
		for (const auto& modelData : orca->models()) {
			for (const auto& modelInstance : modelData.mInstances) {
				...
				bottomLevelAccStructures.push_back(
					cgb::bottom_level_acceleration_structure_t::create(model->pos)
				)
			}
		}
		...
		for (const auto& pair : distinctMaterialsOrca) {
			...
			for (const auto& indices : pair.second) { // pair.second = all models and meshes which use the material config (i.e. pair.first)
				...
				auto indexTexelBuffer = cgb::create_and_fill(
					...
					cgb::memory_usage::device,
					indicesData.data()
				); vk::MemoryBarrier{}.setSrcAccessMask(vk::AccessFlagBits::)
